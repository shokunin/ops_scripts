...
func init() {
	...
	flag.StringVar(&prefix, "prefix", "NOWAYISTHISGOINGTOWORK", "prefix to delete")
	...
	flag.IntVar(&batch_size, "batch-size", 1000000, "Batch size to use default is 1M")
	flag.Parse()
}

func main() {

	keys := fetchAllKeys(hostname, port, database, prefix)
	fmt.Println("DELETING ", batch_size, " keys out of ", len(keys))
	if len(keys) < batch_size {
		fmt.Println("Batch size is greater than the number of keys")
		fmt.Println("Setting batch size to the number of keys")
		batch_size = len(keys)
	}
