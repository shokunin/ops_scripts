...
func init() {
	...
	flag.IntVar(&dst_database, "dst_database", 1, "Redis database to migrate to.  DB 1 is the default")
	flag.Parse()
}
...
func main() {
    ...
    for w := 0; w <= concurrent; w++ {
        go worker(w, jobs, results, hostname, port, database, dst_database)
    }
