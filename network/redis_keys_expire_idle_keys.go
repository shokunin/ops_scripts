...
func worker(id int, jobs <-chan string, results chan<- string, hostame string, port int, database int, maxIdle int) {
	re := regexp.MustCompile("lru_seconds_idle:([0-9]+)")
...
func init() {
	...
	flag.IntVar(&maxIdle, "max-idle", 259200, "Number of idle seconds as the threshold for key purge.  259200(3days) is the default")
	flag.Parse()
}
...
func main() {
	...
	for w := 0; w <= concurrent; w++ {
		go worker(w, jobs, results, hostname, port, database, maxIdle)
	}
